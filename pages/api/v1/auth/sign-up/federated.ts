...
export default function federatedSignUpHandler(
  ...
  if(req.method === 'POST'){
    auth.createUser((req.body as body).email, (req.body as body).role)
    .then(() => {
      auth.createSessionWithFederation((req.body as body).oAuthToken, (req.body as body).oAuthProvider)
      .then(session=> res.send(createServerMessage<SessionDto>(session,req)))
