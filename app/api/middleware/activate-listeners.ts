...
import { onDonationClaimAcknowledged, onDonationClaimDeclined } from "../event-listeners/update-cause-on-donation.listener";

export function attachAppEventListeners(req: NextApiRequest, res: NextApiResponse, next: CallNextHandler){
    const emitter = EventRepo.getRepo().emitter;
    /**Donation events */
    emitter.on(EventType.DONATION_CLAIM_ACKNOWLEDGED, onDonationClaimAcknowledged);
