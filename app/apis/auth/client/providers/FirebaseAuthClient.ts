...
import {Auth, getAuth,signInWithEmailAndPassword, signInWithCustomToken, UserCredential} from 'firebase/auth';
...
export default class FirebaseAuthClient implements IAuthClient{
    ...
    private async firebaseSignInWithCustomToken(token: string): Promise<SessionDto> {
        try {            
            const {data} =  ( await axios.post(apis.v1.auth.login.firebase["custom-token"].path,{},{
                 headers: {
                     'authorization': token
                 }
             })).data as ServerMessageDto<SessionDto>;
             if(data){
                return signInWithCustomToken(this.auth,data?.accessToken)
                .then(res => {
                    if(res.user){
